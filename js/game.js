...
class QuoridorGame {
    ...
    reset() {
        ...
        this.horizontalWalls = Array(8).fill(null).map(() => Array(8).fill(false));
        // verticalWalls[x][y] = true if a wall is placed vertically at intersection (x, y)
        this.verticalWalls = Array(8).fill(null).map(() => Array(8).fill(false));
    ...
    isBlockedByWall(x1, y1, x2, y2) {
        ...
        if (y1 === y2) {
            ...
            const vWall1 = leftX >= 0 && leftX < 8 && y1 >= 0 && y1 < 8 && this.verticalWalls[leftX][y1];
            const vWall2 = leftX >= 0 && leftX < 8 && (y1 - 1) >= 0 && (y1 - 1) < 8 && this.verticalWalls[leftX][y1 - 1];
        ...
        if (x1 === x2) {
            ...
            const hWall1 = x1 >= 0 && x1 < 8 && topY >= 0 && topY < 8 && this.horizontalWalls[x1][topY];
            const hWall2 = (x1 - 1) >= 0 && (x1 - 1) < 8 && topY >= 0 && topY < 8 && this.horizontalWalls[x1 - 1][topY];
    ...
    getValidMoves(playerNum = this.turn) {
        ...
        directions.forEach(dir => {
            ...
            if (nx === opponent.x && ny === opponent.y) {
                ...
                if (this.isValidSquare(jumpX, jumpY) && !this.isBlockedByWall(nx, ny, jumpX, jumpY)) {
                    ...
                    const diags = dir.dx !== 0 
                        ? [{ dx: 0, dy: 1 }, { dx: 0, dy: -1 }] // Horizontal jump blocked -> check vertical branches
                        : [{ dx: 1, dy: 0 }, { dx: -1, dy: 0 }]; // Vertical jump blocked -> check horizontal branches
    ...
    movePawn(tx, ty) {
        ...
        if (active.y === active.goalY) {
            ...
            this.logs.push(`🎉 Player ${this.turn} reached the goal! Winner announced!`);
        } else {
            // Next turn
    ...
    placeWall(x, y, type) {
        ...
        if (!this.canPlaceWallSyntax(x, y, type)) {
            this.logs.push(`Wall placement at (${x + 1}, ${y + 1}) [${type}] overlaps with an existing wall.`);
            return false;
        }

        // 3. Verify wall doesn't trap any player
        if (this.wouldBlockPath(x, y, type)) {
            this.logs.push(`Wall placement at (${x + 1}, ${y + 1}) [${type}] blocks a player's only path to the goal.`);
            return false;
        }
        ...
        this.logs.push(`Player ${this.turn} placed a ${type === 'H' ? 'Horizontal' : 'Vertical'} wall at ${coordsStr}`);
